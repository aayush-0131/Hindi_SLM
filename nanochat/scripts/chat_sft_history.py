...
# FP16_T4_MASTER_PARAM_FIX
# Stable-33750 was trained in BF16, and assign=True preserves checkpoint
# parameter dtypes. GradScaler on the T4 FP16 path requires FP32 master
# parameters. Forward computation still casts to COMPUTE_DTYPE.
if COMPUTE_DTYPE == torch.float16:
    promoted_tensors = 0
    promoted_params = 0
    for p_ in model.parameters():
        if p_.dtype in (torch.float16, torch.bfloat16):
            promoted_tensors += 1
            promoted_params += p_.numel()
            p_.data = p_.data.float()
    print0(
        f"FP16 master-param fix: promoted "
        f"{promoted_tensors} tensors / {promoted_params:,} params to FP32"
    )
