...
# MEMORY_EFFICIENT_PARTIAL_SFT
# T4 fallback after measured full-SFT optimizer OOM.
# Freeze the complete base model, then train only the final
# four transformer blocks and the LM head.
for p_ in model.parameters():
    p_.requires_grad_(False)

for block_ in model.transformer.h[-4:]:
...
total_params_ = sum(p_.numel() for p_ in model.parameters())
...
if bad_trainable_dtypes_:
    raise RuntimeError(
        f"Partial-SFT trainable parameters must be FP32 for GradScaler: "
        f"{bad_trainable_dtypes_[:10]}"
    )

print0(
    f"Partial SFT: {trainable_params_:,} / {total_params_:,} "
    f"parameters trainable "
    f"({100.0 * trainable_params_ / total_params_:.2f}%)"
)
print0("Partial SFT scope: final 4 transformer blocks + lm_head")
